...
2. Isolate tests: Run only the failing tests separately to minimize noise and
   focus on the specific scenario.
...
7. Test hypothesis: Methodically test the hypothesis, making small, incremental
   changes and documenting each step.
...
9. If the hypothesis is correct, make the minimal changes to the code to fix the
   error.
10. Verify: Run the full test suite to confirm the fix and ensure no regressions.
