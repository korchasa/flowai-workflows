...
- PM-stage SRS persistence failure is a recurring pattern (issues #147–#183). Always check if SRS file is in diff immediately. This pattern extends to engine scope (requirements-engine.md) too. 25 consecutive failures; resolved in iter 2 for each issue.
...
- Double-v version string bug pattern: when CI passes `github.ref_name` (e.g., "v1.2.3") as VERSION env and getVersionString() prepends "v", the result is "auto-flow vv1.2.3". Check for leading-v stripping in compile scripts.
